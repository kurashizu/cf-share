...
| GET | `/?tunnel=:code` | `src/lib/TunnelRoom.svelte`, rendered from `+page.svelte` | Tunnel room — join form, then the live chat view. No separate route: `/` renders `TunnelRoom` when `?tunnel=` is present, restoring the normal send/receive layout when it's cleared. Creation is the `tunnel` tab in the send panel; joining an existing code also works from the home page's code-entry box |
