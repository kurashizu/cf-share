...
const putResult = await c.send(new PutObjectCommand({ Bucket: bucket, Key: "debug-marker.txt", Body: body, ContentType: "text/plain" }));
console.log("PUT ETag from SDK:", putResult.ETag);
try {
  const h2 = await c.send(new HeadObjectCommand({ Bucket: bucket, Key: "debug-marker.txt" }));
